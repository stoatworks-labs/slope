...
test card and reports any that made no difference at all.
    ...
    python3 tools/sweep.py [--binary build/sltest] [--size WxH] [--jobs N] [--allow-no-gl]

Exit code 1 means something is dead.

------------------------------------------------------------------ the traps

**The card has to have edges and change.** A delta modulator on a flat field
is a two-sample dither and little else; the card carries colour bars (hard
edges in every channel), a ramp, a block that cuts and a moving bar, so a
step-size or adaptation change has somewhere to show. `sltest --out` renders
it for 40 frames.

**Some controls need another to mean anything.** Max Step and Run Length only
matter while the step adapts, which the default does; Bit Errors at its top
end is one flipped bit in ten and cannot be missed. CONTEXT says what else
has to be true for each, and uses nothing but the plugin's own parameters.
...
`sltest --list` prints the count as the range, and the ends swept are the
first and last element; Pixels/Sample lists 1..16 and is swept at both.
...
CONTEXT = {
    "Max Step": {"Adaptation Rate": 1},
    "Run Length": {"Adaptation Rate": 0.25},
}
...
def main():
    ...
    ap.add_argument("--binary", default=str(ROOT / "build" / "sltest"))
    ...
    scratch = tempfile.mkdtemp(prefix="slsweep")

    # One render first: a runner with no GL at all cannot sweep anything, and
    # that has to read as a skip, loudly, not as twelve dead controls.
